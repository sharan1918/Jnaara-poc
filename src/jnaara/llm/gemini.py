...
class GeminiProvider(LLMProvider):
    """Secondary LLM provider using Google Gemini and LangChain's ChatGoogleGenerativeAI integration."""
    ...
    def extract_claims(self, fact: Fact) -> FactAnalysis:
        ...
        system_prompt = (
            ...
            "normalized numeric value (if quantitative), unit, temporal scope, claim_type, and confidence (0.0 to 1.0). "
    ...
    def analyze_inference_conflict(
        ...
        system_prompt = (
            ...
            "Do not flag obvious direct quantitative differences (handled deterministically). "
            "Focus on logical incompatibilities across multiple statements.\n"
            "If a contradiction exists, set is_conflict=True, specify conflict_type, severity, "
            "list related_belief_ids, and explain the logical conflict clearly."
        )
        beliefs_text = "\n".join(
